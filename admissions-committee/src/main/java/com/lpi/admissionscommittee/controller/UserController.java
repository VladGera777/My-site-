...
public class UserController {
    ...
    public String registerUser(@Valid @ModelAttribute("user") UserDto userDto, BindingResult result,
        ...
        try {
            ...
            redirectAttributes
                    .addFlashAttribute(Constants.ERROR,
        ...
        if (!result.hasErrors() && userDto != null && userDto.getUsername().equalsIgnoreCase(username)) {
            try {
                ...
                redirectAttributes.addFlashAttribute
                        (Constants.ERROR, "An account for that email already exists.");
                log.warn("User already exists", e);
                return "redirect:/user/"+username+"/edit";
            }
            redirectAttributes.addAttribute(Constants.MESSAGE, "Your changes have been submitted");
        } else {
            log.warn("Invalid UserDto: " + (userDto == null ? "value is null" : userDto.toString()));
            redirectAttributes.addAttribute(Constants.ERROR, "Some fields have incorrect values, please try again.");
        }
        return Constants.REDIRECT_EDIT_ACCOUNT;
    }
    /**
    ...
    public String getAccountForm(Model model, @PathVariable("username") String username) {
        UserDto userDto = userService.findByUsername(username);
        if (!model.containsAttribute(Constants.USER_DTO)) {
            model.addAttribute(Constants.USER_DTO, userDto);
        }
        return Constants.EDIT_ACCOUNT;
    }
