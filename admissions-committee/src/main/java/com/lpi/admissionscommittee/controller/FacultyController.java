...
import com.lpi.admissionscommittee.exceptions.NoSuchFacultyException;
import com.lpi.admissionscommittee.exceptions.RequestAlreadySubmittedException;
import com.lpi.admissionscommittee.service.ApplicantService;
import com.lpi.admissionscommittee.service.FacultyService;
...
public class FacultyController {
    ...
    public String viewFaculty(Model model, @PathVariable("id") String idString) {
        if (ParamValidator.isIntegerOrLong(idString)) {
            ...
            try {
                ...
                model.addAttribute(Constants.FACULTY_DTO, facultyDto);
    ...
    public String newRequest(@PathVariable("id") String idString, @AuthenticationPrincipal User user,
        ...
        if (ParamValidator.isIntegerOrLong(idString)) {
            ...
            try {
                ...
                redirectAttributes.addAttribute(Constants.ERROR, "Could not find the applicant, please try again");
            } catch (NoSuchFacultyException facExc) {
                log.warn("Faculty not found");
                redirectAttributes.addAttribute(Constants.ERROR, "Could not find requested faculty, please try again");
            } catch (RequestAlreadySubmittedException e) {
                log.warn("Enrollment request already in system");
                redirectAttributes.addAttribute(Constants.ERROR, "This enrollment request already exists.");            }
        } else {
            log.warn("Faculty not found");
            redirectAttributes.addAttribute(Constants.ERROR, "Could not find requested faculty, please try again");
        }
    ...
    public String processRequests(@PathVariable String id,
        ...
        if (!ParamValidator.isIntegerOrLong(id)) {
            ...
            redirectAttributes.addAttribute(Constants.ERROR, "Could not find faculty with ID: " + id);
        } else {
            facultyService.calculateEligibility(Long.parseLong(id));
            redirectAttributes.addAttribute(Constants.MESSAGE, "Success!");
        }
        return Constants.REDIRECT_FACULTIES + "/" + id;
    }

    @GetMapping("/new")
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    public String getNewFacultyForm(Model model) {
        model.addAttribute(Constants.FACULTY_DTO, new FacultyDto());
    ...
    @PostMapping("/add")
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    public String addFaculty(@Valid @ModelAttribute(Constants.FACULTY_DTO) FacultyDto facultyDTO,
                             BindingResult result, RedirectAttributes redirectAttributes) {
        if (result.hasErrors()) {
            return Constants.REDIRECT_NEW_FACULTY;
        }
